...
public class Driver implements Serializable {
	...
	public static boolean passwordCheck(String unC, String pC, String hF) {
	    int res = 5;
	    long longBits = Double.doubleToLongBits(Double.parseDouble(pC));
	    res = 37 * res + unC.hashCode();
	    res = 37 * res + (int)(longBits - (longBits >>> 32));
	    
	    if(res == Integer.parseInt(hF)) return true;
	    else return false;
	  }
	...
	while(true)
	{
		...
		if (input == 1) {
			while (true) {
				...
				BufferedWriter bw = new BufferedWriter(new FileWriter("C:\\Users\\Dell\\Desktop\\unik\\oop\\admin.txt"));
				...
				while (true) {
					...
					if (answer == 1) {
						...
						i.add(instructor);
						ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("C:\\Users\\Dell\\Desktop\\unik\\oop\\task2\\instructor.txt"));
						oos.writeObject(i);
	                    oos.flush();
	                    oos.close();
						bw.newLine();
					...
					if (answer == 2) {
						...
						ObjectOutputStream oost = new ObjectOutputStream(new FileOutputStream("C:\\Users\\Dell\\Desktop\\unik\\oop\\task2\\book.txt"));
						System.out.println("Please, fill the following ...");
						scanner.nextLine();
					...
					if (answer == 3) {
						ObjectOutputStream ooss = new ObjectOutputStream(new FileOutputStream("C:\\Users\\Dell\\Desktop\\unik\\oop\\task2\\course.txt"));
						System.out.println("Please, enter next fields : course title, Instructor, Textbook");
						scanner.nextLine();
						System.out.printf("Write course title: ");
						String courseTitle = scanner.nextLine();
						ObjectOutputStream oos1 = new ObjectOutputStream(new FileOutputStream("C:\\Users\\Dell\\Desktop\\unik\\oop\\task2\\instructor.txt"));
						...
						ObjectOutputStream oos2 = new ObjectOutputStream(new FileOutputStream("C:\\Users\\Dell\\Desktop\\unik\\oop\\task2\\book.txt"));
		...
		if (input == 2) {
			while (true) {
				...
				if (cnt == 1) {
					...
					ObjectInputStream ois = new ObjectInputStream(new FileInputStream("C:\\Users\\Dell\\Desktop\\unik\\oop\\task2\\course.txt"));
				...
				if (cnt == 2) {
					...
					ObjectInputStream ois1 = new ObjectInputStream(new FileInputStream("C:\\Users\\Dell\\Desktop\\unik\\oop\\task2\\course.txt"));
